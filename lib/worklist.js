...
export function buildWorklist(patients, { filters = [], search = "", today = todayISO() } = {}) {
  ...
  return buildWorklistFromRows(rows, { filters, search });
}

/**
 * Filters, searches and sorts rows that have already been built.
 *
 * Split out from buildWorklist so the rows can be computed on the server —
 * where the patient records live — while the filtering a clinician toggles in
 * the list stays instant and local. Both paths run the same code, so the
 * worklist cannot mean two different things depending on where it was built.
 */
export function buildWorklistFromRows(rows, { filters = [], search = "" } = {}) {
