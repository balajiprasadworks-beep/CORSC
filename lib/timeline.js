...
import { phaseTransitions } from "@/lib/treatment-course";
...
export const EVENT_KINDS = {
  ...
  phaseTransition: { label: "Treatment phase change", dot: "bg-violet-500", chip: "bg-violet-50 text-violet-700" },
...
export function buildPatientTimeline(patient, encounter) {
  ...
  /* Every phase change the treatment course has actually gone through — the
     exact fact spec item #5 asks the timeline to record, in the clinician's
     own words rather than a bare "AC to TH" that means nothing without the
     regimen memorised. */
  phaseTransitions(patient?.treatmentCourse).forEach((transition, index) => {
    events.push({
      id: `phase-transition-${index}-${transition.to.id}`,
      kind: "phaseTransition",
      date: transition.on,
      title: `Moved from ${transition.from.name} to ${transition.to.name}`,
