...
export const INVESTIGATIONS = [
  { id: "ecg", label: "ECG", unit: "", group: "Cardiac" },
  ...
  { id: "gls", label: "Global Longitudinal Strain (GLS)", unit: "%", group: "Cardiac", trend: true, direction: "lowerWorse", magnitude: true },
  { id: "lvef", label: "LVEF", unit: "%", group: "Cardiac", trend: true, direction: "lowerWorse" },
  { id: "troponin", label: "Troponin", unit: "ng/L", group: "Biomarker", trend: true, direction: "higherWorse" },
  { id: "ntprobnp", label: "NT-proBNP / BNP", unit: "pg/mL", group: "Biomarker", trend: true, direction: "higherWorse" },
...
export const HISTORY_GROUPS = [
  ...
  },
  {
    ...
    checklist: [
      ...
      { id: "af", label: "Atrial fibrillation or other arrhythmia" },
      { id: "valve", label: "Valvular heart disease" },
...
export function computeAlerts(patient, visit) {
  const alerts = [];
  ...
  const troponinField = inv.troponin || {};
  const troponinElevated = /elev/i.test(troponinField.interp || "") || /elev/i.test(troponinField.result || "");
  const symptomatic = symptoms.some((s) =>
  ...
  if (!isNaN(currentLVEF) && patient.baselineLVEF) {
    ...
    if (!isNaN(base) && base - currentLVEF >= 10 && currentLVEF < 53) {
      alerts.push({
        ...
        desc: `LVEF fall of ${(base - currentLVEF).toFixed(0)} points to ${currentLVEF}% (< 53%). Start ACEi/ARB + beta-blocker; repeat echocardiography in 3–4 weeks; consider treatment hold pending cardio-oncology review.`,
      });
    }
  }
