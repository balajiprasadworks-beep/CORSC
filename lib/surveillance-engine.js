...
import { activePhaseTherapyClasses, isStructured } from "@/lib/treatment-course";
...
export function getDynamicSurveillanceTasks(patient, visit) {
  ...
  /**
   * Which classes are being actively given right now, as opposed to given at
   * some point in this course.
   *
   * `therapies` is the union across every phase and stays the input to the
   * baseline proforma, the exposure ledger and every post-treatment check
   * below — cumulative anthracycline risk and the need for a 12-month echo do
   * not end when the anthracycline phase does.
   *
   * The escalating on-treatment schedule is different: it exists to catch
   * toxicity from what is currently infusing, and continuing to demand a
   * troponin every cycle for a drug the patient finished months ago is not
   * caution, it is noise that trains a clinician to stop reading the plan.
   * A patient with no structured course has only ever had one undivided
   * phase, so `activeTherapies` equals `therapies` and nothing here changes
   * for them.
   */
  const activeTherapies = isStructured(patient?.treatmentCourse)
    ? activePhaseTherapyClasses(patient.treatmentCourse)
    : therapies;
  ...
  scored.forEach((therapy) => {
    ...
    else if (activeTherapies.includes(therapy)) addTreatmentTasks(tasks, therapy, risk, cycle ?? 1, patient);
  });
