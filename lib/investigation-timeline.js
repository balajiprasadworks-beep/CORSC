...
import { INVESTIGATIONS, TRENDED_INVESTIGATIONS, hasTherapy } from "@/lib/clinical-data";
import { PHASES, cycleOf, journeyPosition, phaseOf } from "@/lib/encounter-phase";
...
/**
 * Milestones use "cycle" where the shared resolver says "treatment"; the two
 * mean the same position in the journey.
 */
function phaseOfVisit(visit) {
  const phase = phaseOf(visit);
  return phase === PHASES.treatment.id ? "cycle" : phase;
}

function visitCycleNumber(visit) {
  return cycleOf(null, visit);
}
...
/**
 * Investigations required at a milestone, across every planned therapy.
 *
 * Each therapy contributes its own protocol and the results are unioned, so a
 * patient on anthracycline followed by trastuzumab is scheduled for both. The
 * previous implementation read only the first therapy.
 */
export function milestoneRequirements(patient, milestone) {
  ...
  (therapies.length ? therapies : [null]).forEach((therapy) => {
    collectRequirements(patient, milestone, therapy, add);
  });

  return requirements;
}

/** The protocol contributed by one therapy at one milestone. */
function collectRequirements(patient, milestone, therapy, add) {
...
function currentPosition(patient, encounter) {
  const { cycle, order } = journeyPosition(patient, encounter);
  return { phase: phaseOfVisit(encounter), cycle: cycle || 1, order };
}
...
export function trendFlags(trends, patient) {
  ...
  trends.forEach((series) => {
    ...
    // Trend flags describe direction of travel only. The graded CTRCD verdict
    // comes from lib/ctrcd.js, which sees symptoms and biomarkers as well.
    if (series.id === "lvef" && series.change <= -10 && series.last < 50) {
      flags.push({ id: "lvef", level: "danger", text: `LVEF has fallen ${Math.abs(series.change)} points to ${series.last}%. See the graded assessment in the risk section.` });
