...
describe("workflow mode on the built picture", () => {
  it("agrees with the Action Bar verdict: routine only when the verdict is proceed", () => {
    const picture = build({ encounter: { ...createEncounter("Cycle 4"), visitType: "cycleReview", symptoms: [], sinceLastVisit: {} } });
    // Whatever this fixture's fitness verdict turns out to be, the workflow
    // mode must be consistent with it — routine only accompanies "proceed";
    // anything else is, correctly, an abnormal-review trigger.
    if (picture.fitness.verdict === "proceed") {
      expect(picture.workflowMode.mode).toBe("routine");
    } else {
      expect(picture.workflowMode.mode).toBe("abnormal");
    }
  });

  it("recommends ABNORMAL for an unscheduled review", () => {
    const picture = build({ encounter: { ...createEncounter("Unscheduled review"), visitType: "unscheduled" } });
    expect(picture.workflowMode.mode).toBe("abnormal");
  });

  it("recommends BASELINE for a baseline visit regardless of findings", () => {
    const picture = build({ encounter: { ...withLvef(30), visitType: "baseline" } });
    expect(picture.workflowMode.mode).toBe("baseline");
  });
});

describe("Action Bar override", () => {
  it("does not mutate the algorithmic verdict when overridden", () => {
    const picture = build({
      patient: {
        overrides: [
          {
            ...
            reason: "Discussed with oncology; proceeding with closer monitoring.",
            clinician: { id: "c1", name: "Dr Test" },
    ...
    // The algorithmic verdict survives untouched — see lib/overrides.js: an
    // override is additive, never a mutation of the computed result.
    expect(picture.fitness.algorithmicVerdict).toBeDefined();
    expect(picture.fitness.overridden).toBe(true);
    expect(picture.fitness.clinicianValue).toBe("proceed");
    expect(picture.fitness.override.reason).toContain("Discussed with oncology");
  });

  it("reports no override when none has been recorded", () => {
    const picture = build();
    expect(picture.fitness.overridden).toBe(false);
    expect(picture.fitness.algorithmicVerdict).toBe(picture.fitness.verdict);
  });

  it("ignores a withdrawn override and falls back to the algorithmic verdict", () => {
    const picture = build({
      patient: {
        overrides: [
          {
...
describe("one definition of cardiac dysfunction", () => {
  ...
  it("escalates the medication engine from the same grade", () => {
    ...
    // The strength vocabulary no longer contains "Indicated". CORSC does not
    // hold the potassium, the creatinine or the allergy history, so it raises a
    // review prompt rather than asserting that a drug should have been given.
    expect(acei.strength).toBe("Review");
    expect(acei.statement).toMatch(/may be appropriate/);
    expect(acei.checkBefore.length).toBeGreaterThan(0);
