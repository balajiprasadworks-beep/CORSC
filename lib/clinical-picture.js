...
import { assessVisitCompleteness } from "@/lib/completeness";
...
import { activePhase, orderedPhases, phasePosition, phaseTransitions } from "@/lib/treatment-course";
...
export function buildClinicalPicture(patient, encounter) {
  ...
  /* Baseline and pre-treatment visits are scored against the full baseline
     dataset; every other visit type against its own minimum dataset, so an
     established patient is never blocked on a baseline item they were never
     going to retype. See lib/completeness.js. */
  const completeness = assessVisitCompleteness(patient, encounter, { visitType, tasks, intervalHistory });
  ...
  /* A phase whose activation date is today's encounter date changed at this
     visit specifically, rather than at some earlier point in the course. */
  const phaseTransitionedToday = Boolean(
    encounter?.date && orderedPhases(patient?.treatmentCourse).some((phase) => phase.activatedOn === encounter.date)
  );
  const workflowMode = resolveWorkflowMode({
    visitType,
    intervalHistory,
    redFlags: redFlags.flags,
    fitnessVerdict: fitness.verdict,
    phaseTransitioned: phaseTransitionedToday,
  });
  ...
  return {
    ...
    /** BASELINE / ROUTINE REVIEW / ABNORMAL REVIEW / SURVIVORSHIP — see lib/workflow-mode.js. */
    workflowMode,
  };
}
