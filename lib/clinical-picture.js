...
export function buildClinicalPicture(patient, encounter) {
  ...
  const alerts = computeAlerts(patient, engineVisit);
  ...
  let currentRisk = patient?.risk?.category || "Low";
  let riskEscalation = null;
  if (restrat.myocarditisConfirmed || restrat.severeHF || signals.lvefDecline) {
    ...
    riskEscalation = restrat.myocarditisConfirmed
      ...
      : restrat.severeHF
        ...
        : "LVEF fall meeting CTRCD criteria";
  } else if (signals.elevatedTroponin || signals.glsDrop || restrat.troponinRise || restrat.symptomatic) {
    if (currentRisk !== "Very High") {
      currentRisk = "High";
      riskEscalation = signals.elevatedTroponin || restrat.troponinRise
