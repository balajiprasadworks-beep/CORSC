...
export function createEncounter(type = "Baseline", options = {}) {
  ...
  return {
    ...
    symptoms: [],
    symptomDetail: {},
    /** Distinguishes "not asked yet" from "asked, and there was nothing new" —
        an empty symptoms array alone cannot tell those apart. */
    symptomsScreenComplete: false,
...
export function encounterVitals(patient, encounter) {
  return deriveVitals(encounter?.vitals || {}, patient?.baselineWeight, patient?.baselineHeight);
}
...
/**
 * Seeds a fresh baseline encounter's LVEF, GLS and weight from the
 * registration baseline values already on file, so the same measurement is
 * not asked for twice — once as "baseline" and again as "today's result".
 * Only fills a field that is genuinely empty; anything already entered for
 * this encounter, or entered at any other visit type, is left untouched.
 */
export function seedBaselineDefaults(encounter, patient) {
  if (!encounter || encounter.type !== "Baseline") return encounter;

  let inv = encounter.inv;
  const seedInv = (id, source) => {
    if (!isFilled(source) || isFilled(inv?.[id]?.result)) return;
    if (inv === encounter.inv) inv = { ...(encounter.inv || {}) };
    inv[id] = { result: "", interp: "", date: patient?.registeredDate || "", comment: "", ...(inv[id] || {}) };
    inv[id].result = source;
  };
  seedInv("lvef", patient?.baselineLVEF);
  seedInv("gls", patient?.baselineGLS);

  let vitals = encounter.vitals;
  if (isFilled(patient?.baselineWeight) && !isFilled(vitals?.weight)) {
    vitals = { ...(encounter.vitals || {}), weight: patient.baselineWeight };
  }

  if (inv === encounter.inv && vitals === encounter.vitals) return encounter;
  return { ...encounter, inv, vitals };
}
