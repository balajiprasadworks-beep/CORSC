...
export function resolveFollowUp({ triggers = [], risk = "Moderate", from, baseDays } = {}) {
  ...
  const resolvedReasons = reasons.length ? reasons : [`${String(risk).toLowerCase()}-risk routine surveillance`];
  ...
  return {
    ...
    /** Single-sentence form, for report text and one-line captions. */
    reason: resolvedReasons.join("; "),
