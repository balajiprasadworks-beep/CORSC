...
describe("therapy class derivation", () => {
  ...
  it("lets a clinician assert a class the library cannot supply", () => {
    // ponatinib is not in the library, so nothing can derive bcrabl for it.
    const agent = createAgent({ genericName: "ponatinib", corscTherapyClass: "bcrabl" });
    expect(corscTherapyClassForAgent(agent)).toBe("bcrabl");
  });

  it("ignores an asserted class that is not a CORSC therapy class", () => {
    const agent = createAgent({ genericName: "ponatinib", corscTherapyClass: "not-a-class" });
